...
type
  ...
  public
    ...
    procedure MarkRootEnv(AEnv: TXuiJsEnv);               // 供绑定引擎标记作用域环境
...
begin
  ...
  begin
    ...
    begin
      ...
      // v1：顶层 + 一层嵌套对象标记
      for i := 0 to src.Obj.Props.Count - 1 do
        if (TXuiJsProp(src.Obj.Props[i]).Value.Kind = jvObject) and
           (TXuiJsProp(src.Obj.Props[i]).Value.Obj <> nil) and
           (not (TXuiJsProp(src.Obj.Props[i]).Value.Obj is TXuiJsFunction)) then
          TXuiJsProp(src.Obj.Props[i]).Value.Obj.Reactive := True;
    end;
    Exit(src);
  end;
